...
func start_server(collector string) {
	...
	log.Printf("start_server: g_ctx: %p", gen_ctx)
...
func (g_ctx *generate_context_t) generate_start(writer http.ResponseWriter, request *http.Request) {
	log.Printf("generate_start called: g_ctx: %p\n", g_ctx)
	if !g_ctx.is_running {
		go generate(g_ctx)
	}
	return
...
func (g_ctx *generate_context_t) generate_stop(writer http.ResponseWriter, request *http.Request) {
	// log.Println("generate_stop called")
	log.Printf("generate_stop called: g_ctx: %p\n", g_ctx)
	if g_ctx.is_running {
		go func() {
			g_ctx.command <- int(0)
		}()
...
func (g_ctx *generate_context_t) get_status(writer http.ResponseWriter, request *http.Request) {
	// log.Println("get_status called")
	log.Printf("get_status called: g_ctx: %p\n", g_ctx)
	if g_ctx.is_running {
